...
// Here is where the external file should be called (Jeff 06_05_20)
// Is it possible to change this based on the kind of particles 
//-----------------------------------------------------------------------------------      
 	double pxi;
 	double pyi;
 	double pzi;
	if ( npt->kind == 2){
//	if ( kinds[kind].q <= 0 && kinds[kind].w == 0.03) { // This is for the kappa distribution for the halo
     pxi = npt->p[0] +
      sqrtf(-2.*2.f*npt->T[0]/npt->m*sqr(beta)*logf(1.0-ran1)) * cosf(2.f*M_PI*ran2);
     pyi = npt->p[1] +
      sqrtf(-2.*2.f*npt->T[1]/npt->m*sqr(beta)*logf(1.0-ran3)) * cosf(2.f*M_PI*ran4);
     pzi = npt->p[2] +
      sqrtf(-2.*2.f*npt->T[2]/npt->m*sqr(beta)*logf(1.0-ran5)) * cosf(2.f*M_PI*ran6);	
		} else {
			     pxi = npt->p[0] +
...
//-----------------------------------------------------------------------------------
// It is important to make the distintion between the ions and electrons when changing 
// the distribution   
//-----------------------------------------------------------------------------------
	//ofstream myfile;
	//myfile.open ("pxyze.txt", ios::in | ios::out);
	//myfile.close();
	//if (0) {
    //double pxi = npt->p[0] +
    //  sqrtf(-2.f*npt->T[0]/npt->m*sqr(beta)*logf(1.0-ran1)) * cosf(2.f*M_PI*ran2);
