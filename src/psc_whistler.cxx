...
//#include "inject_impl.hxx" //This one doesn't work interfere with marder
...
struct PscWhistler : Psc<PscConfig>
{
  ...
  PscWhistler()
  {
    ...
    // -- output particles
    //OutputParticlesParams outp_params; //It works with and without the {}
    //--psc_output_particles_every_step 100

    //outp_params.every_step = 400; //400;
    //outp_params.data_dir = "/home/dc-agud1/PSC/psc-dev/JOBS/Whistler_checkpoint"; //".";
    //outp_params.basename = "prt";
    //psc_output_particles_hdf5 outp{grid, outp_params};
    
    // OutputParticles outp{grid, outp_params};
    //PscOutputParticles<OutputParticles_t> outp_params(_outp);   
    
    // -- output particles
    //psc_output_particles_set_type(psc->output_particles,"hdf5");
    //psc_output_particles_set_param_bool(psc->output_particles, "x, y, x, px, py, pz, charge, mass, weight" , true);
    //psc_output_particles_set_param_int(psc->output_particles, "particle_step" , 500);
...
private:
  void init_npt(int kind, double crd[3], psc_particle_npt& npt)
  {
	...
	// Maybe call here the external file to load the random numbers
	
	ofstream myfile;
	myfile.open ("/Users/jeffersson_agudelo/Documents/PSC_here/PSC_This_mac/psc-dev/electron_files/pxyze.txt", ios::in | ios::out);
